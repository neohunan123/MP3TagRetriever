...
public class ID3v1 extends AbstractID3v1 {
    ...
    public void read(final RandomAccessFile file) throws TagNotFoundException, IOException {
        ...
        this.title = new String(buffer, 0, 30, "GBK").trim();
        file.read(buffer, 0, 30);
        this.artist = new String(buffer, 0, 30, "GBK").trim();
        file.read(buffer, 0, 30);
        this.album = new String(buffer, 0, 30, "GBK").trim();
        file.read(buffer, 0, 4);
        this.year = new String(buffer, 0, 4, "GBK").trim();
        file.read(buffer, 0, 30);
        this.comment = new String(buffer, 0, 30, "GBK").trim();
